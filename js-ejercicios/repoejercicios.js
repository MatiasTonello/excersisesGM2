...
function imprimirResultadoDelInput(valor, number){
    opcionSeleccionada = listEjerc.selectedIndex;
    resultadoContainer.style.display = "block";
    if(opcionSeleccionada === 1){
        let arrayToAdd = valor.split(",").map(Number);
        let numberToNumber = Number(number); 
        console.log(typeof numberToNumber);
        if(document.getElementById("addcbox").checked){
        resultadoContainer.innerHTML = "<p> EL resultado de la adición es " + addition(arrayToAdd, numberToNumber) + "</p>";
        } 
        if(document.getElementById("restcbox").checked){
        resultadoContainer.innerHTML = "<p> EL resultado de la resta es " + rest(arrayToAdd, number) + "</p>";
        }
    }
    if(opcionSeleccionada === 2){
    ...
    if(opcionSeleccionada === 4){
    resultadoContainer.innerHTML = "<p> El número convertido es : " + (convertirARomano(valor) + "</p>");
    }
